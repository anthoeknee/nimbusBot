  ...
  execute: commandErrorHandler(async (interactionOrMessage: ChatInputCommandInteraction | Message, context?: { args?: string[] }) => {
    try {
      if ("options" in interactionOrMessage) {
        ...
        if (!subcommand) {
          throw new Error("No subcommand provided. Use: browse, edit, or create");
        }
        ...
        switch (subcommand) {
          case "browse":
            ...
            break;
          case "edit":
            if (!args[1]) throw new Error("File path is required for edit command");
            await handleEditPrefix(interactionOrMessage, codeEditor, args[1]);
            break;
          case "create":
            if (!args[1]) throw new Error("File path is required for create command");
            if (!args[2]) throw new Error("File type is required for create command");
            await handleCreatePrefix(interactionOrMessage, codeEditor, args[1], args[2]);
            break;
          default:
            throw new Error(`Unknown subcommand: ${subcommand}`);
        }
      ...
      if ("reply" in interactionOrMessage) {
        if ("options" in interactionOrMessage && !interactionOrMessage.replied && !interactionOrMessage.deferred) {
          await interactionOrMessage.reply({
            content: `❌ An error occurred: ${error.message}`,
            ephemeral: true
          });
        } else {
          await (interactionOrMessage as any).reply(`❌ An error occurred: ${error.message}`);
        }
      }
    }
  })
...
async function handleBrowse(interaction: ChatInputCommandInteraction, codeEditor: CodeEditorService) {
  try {
...
async function handleEdit(interaction: ChatInputCommandInteraction, codeEditor: CodeEditorService) {
  try {
...
async function handleCreate(interaction: ChatInputCommandInteraction, codeEditor: CodeEditorService) {
  try {
...
async function handleBrowsePrefix(message: Message, codeEditor: CodeEditorService, targetPath?: string) {
  try {
...
async function handleEditPrefix(message: Message, codeEditor: CodeEditorService, filePath: string) {
  try {
...
async function handleCreatePrefix(message: Message, codeEditor: CodeEditorService, filePath: string, fileType: string) {
  try {
    const template = await codeEditor.getTemplate(fileType);
    
    const embed = new EmbedBuilder()
      .setTitle("📝 Create File")
      .setDescription(`To create a file in \`${filePath}\` with type \`${fileType}\`, please use the slash command:\n\`/code create path:${filePath} type:${fileType}\``)
      .addFields({
...
function setupComponentCollector(message: any, codeEditor: CodeEditorService, currentPath: string) {
  const collector = message.createMessageComponentCollector({
    filter: (i: any) => i.user.id === message.interaction?.user.id || i.user.id === message.author?.id,
    time: 300000 // 5 minutes
  });

  collector.on("collect", async (componentInteraction: any) => {
    try {
      console.log("Component interaction received:", componentInteraction.customId);
      ...
      if (componentInteraction.customId.startsWith("edit_modal_")) {
        const filePath = decodePathFromCustomId(componentInteraction.customId.replace("edit_modal_", ""));
        ...
        const filePath = decodePathFromCustomId(componentInteraction.customId.replace("rename_", ""));
      ...
      if (componentInteraction.customId === "navigate_directory") {
        ...
        const newPath = decodePathFromCustomId(selectedValue.replace("navigate_", ""));
        ...
        const filePath = decodePathFromCustomId(selectedValue.replace("edit_", ""));
        ...
        const newPath = decodePathFromCustomId(componentInteraction.customId.replace("navigate_", ""));
        await handleNavigation(componentInteraction, codeEditor, newPath);
      } else if (componentInteraction.customId.startsWith("edit_")) {
        const filePath = decodePathFromCustomId(componentInteraction.customId.replace("edit_", ""));
        await handleFileEdit(componentInteraction, codeEditor, filePath);
        ...
        const filePath = decodePathFromCustomId(componentInteraction.customId.replace("delete_", ""));
        ...
        const filePath = decodePathFromCustomId(componentInteraction.customId.replace("confirm_delete_", ""));
...
async function handleNavigation(interaction: any, codeEditor: CodeEditorService, newPath: string) {
  try {
...
async function handleFileEdit(interaction: any, codeEditor: CodeEditorService, filePath: string) {
  try {
    const fileContent = await codeEditor.getFileContent(filePath);
    ...
    if (interaction.deferred) {
      ...
      await interaction.followUp({ embeds: [embed], components: [actionButtons], ephemeral: true });
    } else {
      await interaction.reply({ embeds: [embed], components: [actionButtons], ephemeral: true });
    }
  } catch (error: any) {
...
async function handleFileAction(interaction: any, codeEditor: CodeEditorService) {
  try {
...
async function handleFileDelete(interaction: any, codeEditor: CodeEditorService, filePath: string) {
  try {
    const embed = new EmbedBuilder()
...
async function createFileTreeComponents(fileTree: any, currentPath: string) {
  ...
  try {
    ...
    if (fileTree.directories && fileTree.directories.length > 0) {
      const directoryOptions = fileTree.directories.slice(0, 25).map((dir: any) => ({
        label: `📁 ${dir.name}`,
        value: `navigate_${encodePathForCustomId(path.join(currentPath, dir.name))}`,
      ...
      components.push(new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(directorySelect));
    }
    ...
    if (fileTree.files && fileTree.files.length > 0) {
      const fileOptions = fileTree.files.slice(0, 25).map((file: any) => ({
        label: `📄 ${file.name}`,
        value: `edit_${encodePathForCustomId(path.join(currentPath, file.name))}`,
      ...
      components.push(new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(fileSelect));
    }
    ...
    const actionButtons = new ActionRowBuilder<ButtonBuilder>()
      .addComponents(
        ...
        new ButtonBuilder()
          ...
          .setEmoji("🔄"),
        new ButtonBuilder()
          .setCustomId(`action_back_${encodePathForCustomId(path.dirname(currentPath))}`)
          .setLabel("Back")
          .setStyle(ButtonStyle.Secondary)
          .setEmoji("⬅️")
    ...
    const errorButton = new ActionRowBuilder<ButtonBuilder>()
      .addComponents(
        new ButtonBuilder()
          .setCustomId(`action_refresh_${encodePathForCustomId(currentPath)}`)
          .setLabel("Refresh")
