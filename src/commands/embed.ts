    ...
    async (interaction: ChatInputCommandInteraction | Message) => {
      ...
      if (!(interaction instanceof ChatInputCommandInteraction)) {
        await (interaction as Message).reply(
          "❌ Please use this command as a slash command.",
        );
        return;
      }
      ...
      const channelSelectInteraction =
        await new Promise<ChannelSelectMenuInteraction | null>((resolve) => {
      ...
      const selectedChannel =
        channelSelectInteraction.channels.first() as GuildTextBasedChannel | null;
      ...
      const modalSubmit = (await interaction
        .awaitModalSubmit({
          time: 120_000,
          filter: (i) =>
            i.user.id === interaction.user.id && i.customId === "embed_modal",
        })
      ...
      while (managingFields) {
        ...
        const fieldButton = await new Promise<ButtonInteraction | null>(
          (resolve) => {
        ...
        if (fieldButton.customId === "embed_add_field") {
          ...
          const addFieldSubmit = await interaction
            .awaitModalSubmit({
              time: 60_000,
              filter: (i) => i.user.id === interaction.user.id,
            })
          ...
          if (addFieldSubmit) {
            ...
            const value =
              addFieldSubmit.fields.getTextInputValue("field_value");
            const inline =
              addFieldSubmit.fields
          ...
          if (selectInteraction) {
            const toRemove = selectInteraction.values
              .map((v) => parseInt(v, 10))
              .sort((a, b) => b - a);
            for (const idx of toRemove) fields.splice(idx, 1);
            await selectInteraction.update({
              content: "Field(s) removed.",
              components: [],
            });
          }
