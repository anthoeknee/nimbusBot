...
function categorizeError(error: Error): ErrorContext {
  ...
  if (message.includes('network') || message.includes('fetch') || message.includes('http') || 
      message.includes('timeout') || message.includes('connection') || name.includes('network')) {
    return {
      type: ErrorType.NETWORK,
      severity: ErrorSeverity.MEDIUM,
      userFriendly: "Network connection issue. Please check your internet and try again.",
      retryable: true,
      code: 'NETWORK_ERROR'
    };
  }
  ...
  if (message.includes('database') || message.includes('prisma') || message.includes('sql') || 
      message.includes('connection') || name.includes('database')) {
    return {
  ...
  if (message.includes('validation') || message.includes('invalid') || message.includes('required') ||
    ...
    return {
      ...
      severity: ErrorSeverity.LOW,
      userFriendly: "Invalid input provided. Please check your request and try again.",
      retryable: false,
...
function logError(error: Error, context: ErrorContext, interaction?: Interaction | Message) {
  const errorReport: ErrorReport = {
    ...
    error,
    context,
    user: interaction && 'user' in interaction ? interaction.user?.id : undefined,
    guild: interaction && 'guildId' in interaction ? interaction.guildId || undefined : undefined,
    channel: interaction && 'channelId' in interaction ? interaction.channelId || undefined : undefined,
    stack: error.stack,
  ...
  const logLevel = context.severity === ErrorSeverity.CRITICAL ? 'error' : 
                   context.severity === ErrorSeverity.HIGH ? 'error' : 'warn';
  ...
  if (process.env.NODE_ENV === 'development') {
    console.error('\x1b[31m%s\x1b[0m', `[ERROR] ${errorReport.id} - ${error.name}: ${error.message}`);
    if (error.stack) {
...
function createErrorEmbed(error: Error, context: ErrorContext, errorId: string): EmbedBuilder {
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(context.userFriendly || "An unexpected error occurred. Please try again later.")
...
function createErrorActions(errorId: string, context: ErrorContext): ActionRowBuilder<ButtonBuilder> {
...
export function commandErrorHandler<T extends ChatInputCommandInteraction | Message>(
  handler: (ctx: T) => Promise<any> | any,
  commandName?: string
) {
...
async function sendErrorResponse(
  ...
  try {
    if ("reply" in ctx && typeof ctx.reply === "function") {
      const isInteraction = "options" in ctx;
      
      if (isInteraction && 'replied' in ctx && 'deferred' in ctx && !ctx.replied && !ctx.deferred) {
        await ctx.reply({
          embeds: [embed],
          components: [actions],
          ephemeral: true
        });
      } else if (isInteraction && 'followUp' in ctx) {
        await ctx.followUp({
          embeds: [embed],
          components: [actions],
          ephemeral: true
    ...
    try {
      if ("reply" in ctx && typeof ctx.reply === "function") {
        ...
        if (isInteraction) {
          await ctx.reply({
            content: `❌ ${context.userFriendly || "An error occurred. Please try again later."}`,
            ephemeral: true
          });
        } else {
          await ctx.reply({
...
export function getErrorStats() {
  const stats = {
    ...
    recent: errorReports.filter(r => Date.now() - r.timestamp.getTime() < 3600000).length, // Last hour
    critical: errorReports.filter(r => r.severity === ErrorSeverity.CRITICAL).length
  };
...
export function clearOldErrorReports(maxAge: number = 86400000) { // 24 hours default
