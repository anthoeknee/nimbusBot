...
export class GuildRepository extends BaseRepository<GuildRow, CreateGuildInput, UpdateGuildInput> {
  protected readonly tableName = 'guilds';

  async findByDiscordId(discordGuildId: DiscordId): Promise<GuildRow | null> {
    try {
  ...
  async updateByDiscordId(discordGuildId: DiscordId, data: UpdateGuildInput): Promise<GuildRow> {
    try {
